...
export function CitizenReportWizard() {
  ...
  const addEvidenceFile = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      if (typeof reader.result !== "string") {
        ...
        update("evidence", [...form.evidence, { mediaUrl: reader.result, caption: evidenceCaption.trim() || file.name, mediaType: file.type || "file" }]);
        setEvidenceCaption("");
      }
  ...
  const submit = async () => {
    ...
    try {
      const response = await fetch("/api/reports", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ reporterId: user?.id ?? null, ...form, severity: form.severity || undefined, affectedPopulationEstimate: form.affectedPopulationEstimate || undefined }) });
  ...
  const next = () => {
    if (step === 1 && (!form.title.trim() || !form.description.trim())) { setError("Add a short title and describe what is happening."); return; }
  ...
  return <div className="mx-auto w-full max-w-5xl px-4 py-8 sm:px-8 lg:py-12">
    ...
    <div className="grid gap-8 lg:grid-cols-[180px_minmax(0,1fr)]">
      ...
      <section className="min-w-0 rounded-2xl border border-border bg-card p-5 sm:p-8">
        ...
        {step === 2 && <div className="space-y-7"><div><h2 className="text-xl font-medium tracking-[-0.03em] text-foreground">Where is it happening?</h2><p className="mt-1 text-sm text-muted-foreground">Location is optional. Add what you know, even if it is only a nearby landmark.</p></div><div className="flex flex-col gap-4 rounded-xl border border-border bg-muted/40 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-lg bg-primary/10 text-primary"><MapPin className="h-5 w-5" /></span><div><p className="text-sm font-medium text-foreground">Use current location</p><p className="text-xs text-muted-foreground">{form.latitude && form.longitude ? `${form.latitude}, ${form.longitude}` : "Coordinates stay private to the review process."}</p></div></div><Button type="button" variant="outline" onClick={captureLocation} disabled={locating} className="gap-2">{locating ? <Loader2 className="h-4 w-4 animate-spin" /> : <MapPin className="h-4 w-4" />}{locating ? "Detecting" : "Detect location"}</Button></div><div className="grid gap-5 sm:grid-cols-2"><label className="space-y-2 text-sm font-medium text-foreground">State or region <Input value={form.state} onChange={(event) => update("state", event.target.value)} placeholder="Optional" /></label><label className="space-y-2 text-sm font-medium text-foreground">District or local area<select value={form.district} onChange={(event) => update("district", event.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm font-normal focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><option value="">Select if known</option>{LOCAL_DISTRICTS.map((item) => <option key={item}>{item}</option>)}</select></label><label className="space-y-2 text-sm font-medium text-foreground">Village, block, landmark <Input value={form.blockOrPanchayat} onChange={(event) => update("blockOrPanchayat", event.target.value)} placeholder="Optional" /></label><label className="space-y-2 text-sm font-medium text-foreground">PIN code <Input value={form.pinCode} onChange={(event) => update("pinCode", event.target.value)} placeholder="Optional" /></label></div><label className="space-y-2 text-sm font-medium text-foreground">Address or location note <Input value={form.formattedAddress} onChange={(event) => update("formattedAddress", event.target.value)} placeholder="Optional landmark or address" /></label></div>}
