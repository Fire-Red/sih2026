...
export function ProfileSettings({ session }: ProfileSettingsProps) {
  ...
  useEffect(() => {
    ...
    if (profile.role === "student" && roleProfile && "institutionName" in roleProfile) {
      ...
      startTransition(() => {
        ...
        setSkills(institutionProfile.capabilities?.join(", ") || "");
      });
    }
  ...
  const saveProfile = async () => {
    ...
    if (profile.role === "institution") roleProfile = { institutionName: organization, departments: department.split(",").map((item) => item.trim()).filter(Boolean), capabilities: skills.split(",").map((item) => item.trim()).filter(Boolean) };
