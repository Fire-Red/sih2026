...
export default function OnboardingPage() {
  ...
  return (
    <div className="grid w-full overflow-hidden rounded-3xl border border-border bg-card shadow-sm lg:grid-cols-[0.85fr_1.15fr]">
      <aside className="hidden bg-foreground p-10 text-background lg:flex lg:min-h-[620px] lg:flex-col lg:justify-between">
        <div>
          <div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-full bg-primary text-sm font-semibold text-primary-foreground">C</span><span className="text-sm font-medium tracking-[-0.02em]">CivicPulse</span></div>
          <div className="mt-24 max-w-sm"><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-background/55">A clear start</p><h1 className="mt-4 text-4xl font-medium leading-[1.04] tracking-[-0.05em]">Set up the workspace around your role.</h1><p className="mt-5 text-sm leading-7 text-background/65">Choose how you will contribute. You can update your profile and add more detail later.</p></div>
        </div>
        <div className="flex items-center gap-2 text-xs text-background/60"><Check className="h-4 w-4 text-primary" /> Only the details you need</div>
      </aside>

      <main className="min-w-0 p-5 sm:p-8 lg:p-12">
        <div className="flex items-center justify-between border-b border-border pb-5"><div><p className="font-mono text-[10px] uppercase tracking-[0.16em] text-primary">Setup {step} of {totalSteps}</p><p className="mt-1 text-xs text-muted-foreground">{step === 1 ? "Choose your role" : step === 2 ? "Add your context" : "Complete your profile"}</p></div><div className="flex gap-1.5" aria-label={`Step ${step} of ${totalSteps}`}>{Array.from({ length: totalSteps }, (_, index) => <span key={index} className={`h-1.5 w-10 rounded-full ${index < step ? "bg-primary" : "bg-muted"}`} />)}</div></div>

        <div className="mt-9">
          {step === 1 && <section><h2 className="max-w-lg text-3xl font-medium leading-tight tracking-[-0.045em] text-foreground">How will you use the platform?</h2><p className="mt-3 max-w-lg text-sm leading-6 text-muted-foreground">This shapes the tools and updates you see first.</p><div className="mt-8 grid gap-3 sm:grid-cols-2">{ROLES.map(({ role, title, description, icon: Icon }) => { const selected = selectedRole === role; return <button key={role} type="button" aria-pressed={selected} onClick={() => setSelectedRole(role)} className={`group flex min-h-36 flex-col justify-between rounded-2xl border p-4 text-left transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${selected ? "border-primary bg-primary/5" : "border-border bg-background hover:border-primary/40 hover:bg-muted/40"}`}><span className={`flex h-9 w-9 items-center justify-center rounded-xl border ${selected ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card text-primary"}`}><Icon className="h-4 w-4" /></span><span><span className="block text-sm font-medium text-foreground">{title}</span><span className="mt-1 block text-xs leading-5 text-muted-foreground">{description}</span></span></button>; })}</div></section>}
