...
export async function POST(request: Request) {
  try {
    ...
    if (!input.success) return NextResponse.json({ success: false, error: "Invalid question." }, { status: 400 });
    ...
    if (!response.ok) return NextResponse.json({ success: false, error: "AI reasoning is currently unavailable." }, { status: 503 });
    ...
    return NextResponse.json({ success: false, error: "AI reasoning is currently unavailable." }, { status: 503 });
  }
}
